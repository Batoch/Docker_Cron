...
# This will echo a message, run the script, and log success or error
CMD="echo Running $SCRIPT_PATH | tee -a /var/log/cron.log; \
if /bin/bash $SCRIPT_PATH 2>&1 | tee -a /var/log/cron.log; then \
  echo $SCRIPT_PATH finished successfully | tee -a /var/log/cron.log; \
else \
  echo ERROR: $SCRIPT_PATH failed with exit code $? | tee -a /var/log/cron.log; \
fi"
